...
public class CheckoutController implements Initializable, Observer {
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
		...
		for(Product p: products) {
			try {
				loadProductRow((ProductInFacility)p);
			} catch (IOException e) {
				EkrutClientUI.chat.display("Error while loading cart row");
				e.printStackTrace();
			}
		}
	 ...
	 * @throws IOException
	 */
	private void loadProductRow(ProductInFacility product) throws IOException {
		...
		cartVBox.getChildren().add(anchorPane);
	}
